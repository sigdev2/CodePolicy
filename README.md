...
## Именование файлов
 ...
 Исходный код необходимо помещать в файлы с кодировкой UTF-8. Если нужны файлы или данные в другой кодировке, то лучше вынести в отдельный файл с указанием кодировки в имени файлы, например *file1.koi8-r.cpp*
...
Для удобства отладки:
 - Каждое выражение должно находиться на отдельной строке
 - Каждое объявление для каждой переменной должно быть на отдельной строке, никаких групповых объявлений
 - Каждое условие должно быть на своей строке, в то время как условный блок следующий за ним должен находиться ниже.
 - Для элементов перечисляющихся в одной строке через запятую или точку запятой

# Блоки кода

Блоки кода оформляются отступами с шакгом в 4 пробела от уровня родителя.

При использовании скобок ()[]{}<>, если вмещается на одну строку, то записывается без пробелов между содержимым и скобками
    
    (1, 2, 3, 4)

Блоку может предшестовать блок операторов объявляющих блок, тогда в случае если однострочный блок не вмещается с ним на одной строке, то его необходимо перенести на следующую строку и сделать 4 пробела.
...
Многострочные блоки необходимо по следующим правилам:
 - Если первая часть блока остающася на строке больше чем одиночный открывающий символ остаётся на строк
